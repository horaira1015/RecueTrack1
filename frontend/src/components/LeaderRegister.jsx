...
// Bangladeshi address data (reuse or update as needed)
...
function LeaderRegister() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:5553/api/leaders/Reg", formDataToSend, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      if (response.status === 201) {
        setSuccessMessage("Leader registered successfully!");
        setFormData({ name: "", email: "", phone: "", password: "", idProof: null });
        setAddress({ district: "", zilla: "", upazilla: "", street: "" });
  ...
  return (
    <Container className="d-flex justify-content-center align-items-center min-vh-100 py-4">
      <Card style={{ width: "100%", maxWidth: "600px", padding: "2rem", boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }}>
        <h2 className="text-center mb-4" style={{ color: "#2c3e50" }}>Leader Registration</h2>
        ...
        <Form onSubmit={handleSubmit}>
            ...
            <Col md={6}>
              <Form.Group className="mb-3">
                ...
                >
                  ...
                  {availableZillas.map((zilla, idx) => (
                    <option key={idx} value={zilla}>{zilla}</option>
                  ))}
                </Form.Select>
            ...
            <Col md={6}>
              <Form.Group className="mb-3">
                ...
                >
                  ...
                  {availableUpazillas.map((upa, idx) => (
          ...
          <div className="d-grid gap-2">
            ...
            <Button variant="outline-secondary" size="lg" onClick={() => navigate("/viewleaders")}>
              View Registered Leaders
            </Button>
          </div>
        </Form>
      </Card>
